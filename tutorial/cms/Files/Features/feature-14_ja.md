...
**Retrieval-Augmented Generation（RAG）** は、エージェントがクエリ時にドキュメントストアから関連コンテンツを検索し、そのコンテンツをコンテキストに追加して根拠のある回答を生成するパターンです。Smart WorkflowはRAGのベクターストアとして**OpenSearch**と統合しています。

RAGを支える2つの組み込みプロセスがあります：

- `createVectorStore` — ドキュメントを取り込み、チャンクに分割し、埋め込みベクターを生成してOpenSearchに保存する
- `openSearchSearch` — エージェントが実行時にセマンティック検索を行うために呼び出す組み込みツール
...
ドキュメントをアップロード → `createVectorStore`呼び出し可能 → チャンクに分割 → 埋め込みモデルがチャンクをベクターに変換 → OpenSearchインデックスに保存。
...
> `tutorial/processes/tutorial/features/Feature14.p.json` でインスコープ・アウトオブスコープのクエリシナリオを示すチュートリアルデモプロセスを参照してください。
> `smart-workflow-demo/processes/Features/RagChatBotDemo.p.json` でOpenSearch接続確認 → ドキュメントアップロード → 埋め込み → チャットインターフェースの完全な4ステップウィザードを参照してください。
