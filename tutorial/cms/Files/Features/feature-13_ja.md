...
- **UserTaskが同じProgramInterfaceに戻る接続がない** — エージェントが再エントリーできず、プロセスが途中で終了する。
- **`tool`タグを忘れる** — `CallSubStart`に`tool`タグがないと、エージェントは`askUserFeedback`を発見したり呼び出したりできない。
- **ツールを使用するようエージェントに指示しない** — システムプロンプトに`askUserFeedback`への言及がないと、エージェントは人間に委任する代わりに自律的に決定しようとする可能性がある。
- **システムプロンプトにオプションを列挙する** — プロンプトで選択肢を提示すると、エージェントがその選択肢を質問文に埋め込む可能性がある。プロンプトは簡潔に：1つの質問をし、フリーテキストを期待する。
